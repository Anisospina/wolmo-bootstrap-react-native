...
import GoogleAnalytics, {
  trackScreenView
} from '@redux-beacon/react-native-google-analytics';
...
export const checkAnalyticsTrackingID = () => {
  if (!Config.ANALYTICS_TRACKING_ID) {
    console.warn(
      'Google Analytics Tracking ID has not been properly initialized'
    );
  }
};
...
const pageView = trackScreenView((action, prevState) => ({
  screenName: getCurrentRouteName(prevState.nav)
}));
